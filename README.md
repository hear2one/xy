...
订阅自检通过本机 `127.0.0.1:443` 验证 Xray → Nginx 链路，保留域名 SNI 并绕过系统代理。启动期间最多尝试 5 次，每次请求超时 5 秒；成功还需与本地订阅文件完全一致。失败时另查 Nginx `127.0.0.1:8003`，用于区分入口和后端问题，后端通过不会掩盖 443 故障。该检查不代表公网 DNS、安全组或客户端访问已验证。出现错误时先检查服务和监听状态，不要直接重跑主安装脚本生成新密钥。
